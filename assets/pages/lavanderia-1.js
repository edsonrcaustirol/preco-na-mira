(()=>{
  ...
  function render(){
    ...
    document.getElementById('grid').innerHTML=list.map(p=>`<article class="laundry-product"><div class="laundry-product-media"><img src="${esc(p.imagem)}" loading="lazy" decoding="async" data-fallback-src="${esc(p.imagemFallback)}" alt="${esc(p.imagemAlt||p.nome)}">${p.imagemTipo==='oficial'?'<span class="official-image-badge">✓ FONTE OFICIAL</span>':''}</div><div class="laundry-product-copy"><small>${esc(p.marca)} • ${esc(p.selo)}</small><h3>${esc(p.nome)}</h3><p>${esc(p.resumo)}</p><div class="laundry-tags">${(p.chips||[]).slice(1).map(x=>`<span>${esc(x)}</span>`).join('')}</div><div class="actions"><a class="btn btn-dark" href="produto-${encodeURIComponent(p.id)}">ANALISAR →</a><a class="btn btn-outline" href="${esc(p.linkAfiliado)}" target="_blank" rel="sponsored nofollow noopener noreferrer">OFERTA</a></div></div></article>`).join('');
