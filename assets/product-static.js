(()=>{
 ...
 function addRichProductInfo(){
  ...
  const rules=[
   ['Potência',/\b(\d{2,5})\s*W\b/i,m=>`${m[1]} W`],['Voltagem',/\b(110|127|220|240)\s*V\b/i,m=>`${m[1]} V`],['Capacidade',/\b(\d+(?:[.,]\d+)?)\s*(L|litros?|kg)\b/i,m=>`${m[1]} ${m[2]}`],['Armazenamento / memória',/\b(\d+)\s*(GB|TB)\b/i,m=>`${m[1]} ${m[2].toUpperCase()}`],['Tamanho',/\b(\d+(?:[.,]\d+)?)\s*(?:pol(?:egadas?)?|\")\b/i,m=>`${m[1]} polegadas`],['Proteção',/\bIP\d{2}\b/i,m=>m[0].toUpperCase()],['Pontos',/\b(\d{1,3})\s*pontos?\b/i,m=>`${m[1]} pontos`],['Bluetooth',/\bBluetooth\s*([0-9.]+)\b/i,m=>`Bluetooth ${m[1]}`],['Autonomia',/\b(?:até\s*)?(\d+(?:[.,]\d+)?)\s*(?:h|horas?)\b/i,m=>`Até ${m[1]} h`],['Resolução',/\b(4K|8K|1080p|1440p|2K)\b/i,m=>m[1].toUpperCase()]
  ];
  ...
  const aboutText=document.createElement('p');aboutText.textContent=[brand&&category?`${name} é um produto da ${brand} na categoria ${category}.`:brand?`${name} é um produto da ${brand}.`:'',base].filter(Boolean).join(' ');about.append(aboutText);
