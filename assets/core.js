(function(){
 ...
 window.PNMCartToast=function(message,action){let box=document.querySelector('.pnm-cart-toast');if(!box){box=document.createElement('div');box.className='pnm-cart-toast';box.setAttribute('role','status');box.setAttribute('aria-live','polite');document.body.appendChild(box)}box.innerHTML='';const span=document.createElement('span');span.textContent=message;box.appendChild(span);if(action){const a=document.createElement('a');a.href='carrinho.html';a.textContent=action;box.appendChild(a)}box.classList.add('show');clearTimeout(window.__pnmCartToastTimer);window.__pnmCartToastTimer=setTimeout(()=>box.classList.remove('show'),2600)};
 ...
 function enhanceImageCoverage(){
   ...
   if(meta){
     ...
     const href=p.imagemFonte||((info.review||info.kind==='ai')?'cobertura-imagens.html#'+(info.kind==='ai'?'ai-review':'manual-review'):'');
     if(href){
   ...
   if(copy && !copy.querySelector('.image-coverage-note') && info.review){
     ...
     box.innerHTML=`<strong>${info.kind==='ai'?'🧠 Imagem criada por IA':'🖼️ Cobertura de imagem em revisão'}</strong><p>${info.note}</p><a href="cobertura-imagens.html#${info.kind==='ai'?'ai-review':'manual-review'}">Abrir lista de revisão →</a>`;
 ...
 function productFromCard(card){const link=card.querySelector('a[href^="produto-"],a[href*="produto.html?id="]');if(!link)return null;const id=idFromHref(link.getAttribute('href'));if(!id||typeof PRODUTOS==='undefined')return null;return PRODUTOS.find(x=>x.id===id)||null}
