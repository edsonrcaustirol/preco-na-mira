...
export const QUERY_DEFINITIONS = Object.freeze({
  ...
  total_product_views: { group: 'c2', description: 'Total de visualizações factuais de produto, inferidas de page_view com page_type product e product_id.', sql: `SELECT SUM(_sample_interval) AS product_views\nFROM ${DATA}\nWHERE blob1 = '${VERSION}' AND index1 = 'page_view' AND blob2 = 'page_view' AND blob4 = 'product' AND blob5 != ''` },
  page_views_by_channel: { group: 'c2', description: 'Page views agregados por canal C1; não representa usuários únicos.', sql: `SELECT blob14 AS channel, SUM(_sample_interval) AS page_views\nFROM ${DATA}\nWHERE blob1 = '${VERSION}' AND index1 = 'page_view' AND blob2 = 'page_view' AND blob14 != ''\nGROUP BY channel\nORDER BY page_views DESC, channel\nLIMIT 20` },
  product_views_by_channel: { group: 'c2', description: 'Visualizações factuais de produto agregadas por canal C1.', sql: `SELECT blob14 AS channel, SUM(_sample_interval) AS product_views\nFROM ${DATA}\nWHERE blob1 = '${VERSION}' AND index1 = 'page_view' AND blob2 = 'page_view' AND blob4 = 'product' AND blob5 != '' AND blob14 != ''\nGROUP BY channel\nORDER BY product_views DESC, channel\nLIMIT 20` },
  page_views_by_landing: { group: 'c2', description: 'Page views atribuídos à primeira landing preservada na sessão C1; não representa entradas únicas.', sql: `SELECT blob13 AS landing, SUM(_sample_interval) AS page_views\nFROM ${DATA}\nWHERE blob1 = '${VERSION}' AND index1 = 'page_view' AND blob2 = 'page_view' AND blob13 != ''\nGROUP BY landing\nORDER BY page_views DESC, landing\nLIMIT 500` },
  product_views_by_landing: { group: 'c2', description: 'Visualizações factuais de produto atribuídas à primeira landing preservada na sessão C1.', sql: `SELECT blob13 AS landing, SUM(_sample_interval) AS product_views\nFROM ${DATA}\nWHERE blob1 = '${VERSION}' AND index1 = 'page_view' AND blob2 = 'page_view' AND blob4 = 'product' AND blob5 != '' AND blob13 != ''\nGROUP BY landing\nORDER BY product_views DESC, landing\nLIMIT 500` },
  product_views_by_product: { group: 'c2', description: 'Visualizações factuais de produto por product_id canônico.', sql: `SELECT blob5 AS product_id, SUM(_sample_interval) AS product_views\nFROM ${DATA}\nWHERE blob1 = '${VERSION}' AND index1 = 'page_view' AND blob2 = 'page_view' AND blob4 = 'product' AND blob5 != '' AND blob5 != 'unknown'\nGROUP BY product_id\nORDER BY product_views DESC, product_id\nLIMIT 1000` },
