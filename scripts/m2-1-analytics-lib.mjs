...
export const QUERY_DEFINITIONS = Object.freeze({
  total_page_views: {
    group: 'metrics',
    description: 'Total de page_view válidos do schema M1, ponderado por amostragem.',
    sql: `SELECT SUM(_sample_interval) AS page_views\nFROM ${DATA}\nWHERE blob1 = '${VERSION}' AND index1 = 'page_view' AND blob2 = 'page_view'`,
  },
  total_affiliate_clicks: {
    group: 'metrics',
    description: 'Total de affiliate_click válidos do schema M1, ponderado por amostragem.',
    sql: `SELECT SUM(_sample_interval) AS affiliate_clicks\nFROM ${DATA}\nWHERE blob1 = '${VERSION}' AND index1 = 'affiliate_click' AND blob2 = 'affiliate_click'`,
  },
  affiliate_clicks_by_product: {
    group: 'metrics',
    description: 'Cliques afiliados por product_id.',
    sql: `SELECT blob5 AS product_id, SUM(_sample_interval) AS affiliate_clicks\nFROM ${DATA}\nWHERE blob1 = '${VERSION}' AND index1 = 'affiliate_click' AND blob2 = 'affiliate_click'\nGROUP BY product_id\nORDER BY affiliate_clicks DESC, product_id\nLIMIT 1000`,
  },
  affiliate_clicks_by_page: {
    group: 'metrics',
    description: 'Cliques afiliados por página/origem.',
    sql: `SELECT blob3 AS page, SUM(_sample_interval) AS affiliate_clicks\nFROM ${DATA}\nWHERE blob1 = '${VERSION}' AND index1 = 'affiliate_click' AND blob2 = 'affiliate_click'\nGROUP BY page\nORDER BY affiliate_clicks DESC, page\nLIMIT 200`,
  },
  affiliate_clicks_by_placement: {
    group: 'metrics',
    description: 'Cliques afiliados por placement.',
    sql: `SELECT blob7 AS placement, SUM(_sample_interval) AS affiliate_clicks\nFROM ${DATA}\nWHERE blob1 = '${VERSION}' AND index1 = 'affiliate_click' AND blob2 = 'affiliate_click'\nGROUP BY placement\nORDER BY affiliate_clicks DESC, placement\nLIMIT 100`,
  },
  affiliate_clicks_by_store: {
    group: 'metrics',
    description: 'Cliques afiliados por loja.',
    sql: `SELECT blob6 AS store, SUM(_sample_interval) AS affiliate_clicks\nFROM ${DATA}\nWHERE blob1 = '${VERSION}' AND index1 = 'affiliate_click' AND blob2 = 'affiliate_click'\nGROUP BY store\nORDER BY affiliate_clicks DESC, store\nLIMIT 100`,
  },
  top_products: {
    group: 'metrics',
    description: 'Top 25 produtos por cliques afiliados.',
    sql: `SELECT blob5 AS product_id, SUM(_sample_interval) AS affiliate_clicks\nFROM ${DATA}\nWHERE blob1 = '${VERSION}' AND index1 = 'affiliate_click' AND blob2 = 'affiliate_click'\nGROUP BY product_id\nORDER BY affiliate_clicks DESC, product_id\nLIMIT 25`,
  },
  top_commercial_pages: {
    group: 'metrics',
    description: 'Top 25 páginas/origens por cliques afiliados.',
    sql: `SELECT blob3 AS page, SUM(_sample_interval) AS affiliate_clicks\nFROM ${DATA}\nWHERE blob1 = '${VERSION}' AND index1 = 'affiliate_click' AND blob2 = 'affiliate_click'\nGROUP BY page\nORDER BY affiliate_clicks DESC, page\nLIMIT 25`,
  },
  events_by_hour: {
    group: 'metrics',
    description: 'Distribuição temporal horária em UTC por tipo de evento.',
    sql: `SELECT formatDateTime(timestamp, '%Y-%m-%d %H:00:00', 'Etc/UTC') AS hour_utc, index1 AS event, SUM(_sample_interval) AS events\nFROM ${DATA}\nWHERE blob1 = '${VERSION}' AND index1 IN ('page_view', 'affiliate_click') AND blob2 = index1\nGROUP BY hour_utc, event\nORDER BY hour_utc DESC, event\nLIMIT 2000`,
  },
  affiliate_click_rate_by_page: {
    group: 'metrics',
    description: 'Taxa de clique afiliado por página; razão entre affiliate_click e page_view da mesma página.',
  ...
  unknown_event_types: {
    group: 'quality',
    description: 'Eventos desconhecidos ou divergência entre index1 e blob2.',
    sql: `SELECT index1 AS event_index, blob2 AS event_blob, SUM(_sample_interval) AS rows\nFROM ${DATA}\nWHERE blob1 = '${VERSION}' AND (index1 NOT IN ('page_view', 'affiliate_click') OR blob2 NOT IN ('page_view', 'affiliate_click') OR index1 != blob2)\nGROUP BY event_index, event_blob\nORDER BY rows DESC\nLIMIT 100`,
  },
  missing_affiliate_click_fields: {
    group: 'quality',
    description: 'Campos obrigatórios ausentes em affiliate_click.',
  ...
  unexpected_affiliate_values: {
    group: 'quality',
    description: 'Loja, placement ou host fora do vocabulário M1 esperado.',
  ...
  schema_incompatibilities: {
    group: 'quality',
    description: 'Linhas incompatíveis com o layout m1-v1 ou com os campos esperados por evento.',
  ...
  possible_technical_duplicates: {
    group: 'quality',
    description: 'Heurística de duplicação: linhas idênticas no mesmo timestamp. Não prova duplicidade sem event/session id.',
    sql: `SELECT timestamp, index1 AS event, blob3 AS page, blob5 AS product_id, blob6 AS store, blob7 AS placement, SUM(_sample_interval) AS rows\nFROM ${DATA}\nWHERE blob1 = '${VERSION}'\nGROUP BY timestamp, index1, blob2, blob3, blob4, blob5, blob6, blob7, blob8, blob9, blob10, blob11, blob12, double1\nHAVING SUM(_sample_interval) > 1\nORDER BY rows DESC, timestamp DESC\nLIMIT 100`,
  },
});
