    ...
    for (const item of source.rows) {
      ...
      if (!output.has(identity)) {
        ...
        for (const candidate of sources) {
          base[candidate.target] = available.has(candidate.key)
            ? (Object.prototype.hasOwnProperty.call(candidate, 'defaultValue') ? candidate.defaultValue : 0)
            : null;
        }
...
function blockStatus(keys, failuresByKey, items) {
  ...
  if (failed === keys.length) return 'error';
...
function commercialSort(a, b) {
  return (finite(b.affiliateClicks) ?? -1) - (finite(a.affiliateClicks) ?? -1)
    || (finite(b.productViews) ?? -1) - (finite(a.productViews) ?? -1)
    || (finite(b.pageViews) ?? -1) - (finite(a.pageViews) ?? -1)
    || String(a.landing || a.product_id || a.channel || '').localeCompare(String(b.landing || b.product_id || b.channel || ''));
}
...
export async function loadCommercialDashboard({ accountId, apiToken, fetchImpl = runtimeFetch } = {}) {
  ...
  settled.forEach((result, index) => {
    ...
    else failures.push(normalizeDiagnostic(result.reason, spec));
  });

  if (!values.size) {
    const error = new Error('Falha em todas as consultas do painel comercial.');
    error.name = 'CommercialDashboardQueryError';
    error.failures = failures;
    throw error;
  }

  const failuresByKey = new Map(failures.map(item => [item.key, item]));
  const available = new Set(values.keys());
  ...
  const origins = mergeRows({
    ...
    sources: [
      ...
      { key: 'clicksByChannel', rows: getRows('clicksByChannel'), field: 'affiliate_clicks', target: 'affiliateClicks' },
    ],
  }).map(item => ({ ...item, clickRatePct: ratioPercent(item.affiliateClicks, item.pageViews) }))
  ...
  const landings = mergeRows({
    ...
    sources: [
      ...
      { key: 'clicksByLanding', rows: getRows('clicksByLanding'), field: 'affiliate_clicks', target: 'affiliateClicks' },
    ],
  }).map(item => ({ ...item, clickRatePct: ratioPercent(item.affiliateClicks, item.pageViews) }))
    .sort(commercialSort).slice(0, TOP_ROWS);
  ...
  const products = mergeRows({
    ...
    sources: [
      ...
      { key: 'ctrByProduct', rows: getRows('ctrByProduct'), field: 'affiliate_click_rate_pct', target: 'eligibleCtrPct', defaultValue: null },
    ],
  }).sort(commercialSort).slice(0, TOP_ROWS);
  ...
  const placements = mergeRows({
    ...
    sources: [
      ...
      { key: 'ctrByPlacement', rows: getRows('ctrByPlacement'), field: 'affiliate_click_rate_pct', target: 'eligibleCtrPct', defaultValue: null },
    ],
  });
  const ctrRows = new Map(getRows('ctrByPlacement').map(item => [String(item.placement), item]));
  for (const item of placements) {
    const ctr = ctrRows.get(item.placement);
    item.eligibleImpressions = ctr ? finite(ctr.commercial_impressions) ?? 0 : null;
    item.eligibleClicks = ctr ? finite(ctr.affiliate_clicks) ?? 0 : null;
  }
  ...
  const summaryKeys = ['pageViews', 'productViews', 'affiliateClicks', 'commercialImpressions'];
  const summaryItems = summaryKeys.filter(key => available.has(key));
  return {
    generatedAtUtc: new Date().toISOString(),
    m31StartUtc: M31_START_UTC,
    period: C2_PERIOD,
    totals: {
      pageViews: getScalar('pageViews', 'page_views'),
      productViews: getScalar('productViews', 'product_views'),
      affiliateClicks: getScalar('affiliateClicks', 'affiliate_clicks'),
      commercialImpressions: getScalar('commercialImpressions', 'commercial_impressions'),
    },
    ...
    placements: { status: blockStatus(['clicksByPlacement', 'impressionsByPlacement', 'ctrByPlacement'], failuresByKey, placements), rows: placements.slice(0, TOP_ROWS) },
    failures,
  };
}
...
export function parseCanonicalProductTitles(source) {
  ...
  try {
    ...
    return new Map(products
      ...
      .map(item => [String(item.id).trim(), String(item.nome || item.id).trim()]));
  } catch {
    return new Map();
  }
}
...
function formatCount(value) {
  ...
  return numeric === null ? '—' : numberFormat.format(numeric);
}

function formatPercent(value) {
  const numeric = finite(value);
  return numeric === null ? '—' : `${percentFormat.format(numeric)}%`;
}

function stateMessage(status, noun) {
  if (status === 'error') return `${noun} indisponível temporariamente por falha de consulta.`;
  if (status === 'partial') return `${noun} parcialmente disponível; uma consulta falhou e os demais dados foram preservados.`;
  if (status === 'empty') return 'Ainda não há dados suficientes para esta dimensão.';
  return '';
}

function renderState(status, noun) {
  const message = stateMessage(status, noun);
  if (!message) return '';
  const kind = status === 'error' ? 'error-state' : status === 'partial' ? 'partial-state' : 'empty-state';
...
function renderTable(title, description, block, columns) {
  const state = renderState(block.status, title);
  const table = block.status === 'error' ? '' : `<div class="table-wrap"><table><thead><tr>${columns.map(column => `<th scope="col">${escapeHtml(column.label)}</th>`).join('')}</tr></thead><tbody>${renderRows(block.rows, columns)}</tbody></table></div>`;
...
function opportunityRows(data, productTitles) {
  ...
  if (data.landings.status !== 'error') {
    ...
    for (const item of trafficWithoutClick) {
      opportunities.push({
        ...
        title: item.landing,
        detail: `${formatCount(item.pageViews)} page views atribuídos à landing e 0 affiliate clicks.`,
      });
    }
  ...
  if (data.products.status !== 'error') {
    ...
    for (const item of viewedWithoutClick) {
      opportunities.push({
        ...
        detail: `${formatCount(item.productViews)} product views e 0 affiliate clicks.`,
      });
    }
  }
  ...
  if (data.placements.status !== 'error') {
    ...
    for (const item of comparable.filter(item => bestCtr > 0 && item.eligibleCtrPct < bestCtr * C2_THRESHOLDS.weakPlacementRatio).slice(0, 2)) {
      opportunities.push({
        label: 'PLACEMENT FRACO',
        title: item.placement,
        detail: `${formatPercent(item.eligibleCtrPct)} CTR elegível com ${formatCount(item.eligibleImpressions)} impressões; abaixo de metade do melhor placement comparável.`,
      });
    }
  }
...
function renderOpportunities(data, productTitles) {
  ...
  if (!items.length) {
    return `<section class="panel-section"><div class="section-heading"><div><h2>Oportunidades / exceções</h2><p>Regras simples: landing ≥ ${C2_THRESHOLDS.landingActivity} page views; produto ≥ ${C2_THRESHOLDS.productViews} product views; placement ≥ ${C2_THRESHOLDS.placementImpressions} impressões elegíveis.</p></div></div><p class="state empty-state" role="status">Dados insuficientes para classificar oportunidades com segurança.</p></section>`;
  }
...
function renderFunnel(data) {
  ...
  return `<section class="panel-section funnel-section"><div class="section-heading"><div><h2>Funil observado</h2><p>Contagens de eventos agregados. Page view não é usuário; affiliate click não é venda.</p></div></div>${renderState(data.summary.status, 'Resumo do funil')}<div class="funnel">
    <div class="funnel-step"><span>PAGE VIEWS</span><strong>${escapeHtml(formatCount(data.totals.pageViews))}</strong><small>atividade observada</small></div>
...
<head>
  ...
  <title>Funil Comercial — Preço na Mira</title>
  <style>
    :root{color-scheme:dark;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#08111f;color:#eef4fb}
    *{box-sizing:border-box}body{margin:0;background:#08111f;color:#eef4fb}main{width:min(1240px,calc(100% - 32px));margin:0 auto;padding:36px 0 64px}
    a{color:#9ac8ff}.eyebrow{margin:0 0 8px;color:#79b8ff;font-size:.78rem;font-weight:800;letter-spacing:.12em;text-transform:uppercase}h1{margin:0;font-size:clamp(2rem,5vw,3.2rem);letter-spacing:-.04em}h2{margin:0;font-size:1.05rem}p{color:#9eabb8;line-height:1.55}.top{display:flex;gap:20px;align-items:flex-end;justify-content:space-between;margin-bottom:20px}.refresh{white-space:nowrap;border:1px solid #2a405a;border-radius:10px;padding:9px 12px;text-decoration:none;background:#101c2d}
    .notice{border:1px solid #2a405a;border-left:3px solid #79b8ff;background:#0e1b2d;border-radius:10px;padding:14px 16px;margin:0 0 16px;color:#cbd5df}.meta{font-size:.86rem;margin:4px 0 0}.status-line{display:flex;gap:8px;flex-wrap:wrap;margin:0 0 18px}.pill{border:1px solid #2a405a;border-radius:999px;padding:6px 9px;font-size:.75rem;color:#b9cbe0}
    .cards{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin:18px 0}.card{border:1px solid #20344d;background:#0e1b2d;border-radius:12px;padding:15px}.card span{display:block;color:#8f9ba8;font-size:.78rem}.card strong{display:block;margin-top:6px;font-size:1.8rem;letter-spacing:-.03em}
...
<body>
<main>
  <header class="top"><div><p class="eyebrow">Preço na Mira · C2</p><h1>Funil Comercial</h1><p class="meta">Uma visão operacional sobre dados agregados reais do Analytics Engine.</p></div><a class="refresh" href="${PANEL_PATH}">Atualizar</a></header>
  <p class="notice"><strong>Leitura correta:</strong> <code>affiliate_click</code> é um clique de saída para o Mercado Livre. Não representa venda, pedido ou comissão. Page view não representa usuário.</p>
  <div class="status-line"><span class="pill">Período: ${C2_PERIOD}</span><span class="pill">Jornada por channel/landing: somente eventos com dimensões C1</span><span class="pill">CTR placement/produto: card/related desde ${escapeHtml(M31_START_UTC.replace('T', ' ').replace('Z', ' UTC'))}</span></div>
  <div class="cards">
    <div class="card"><span>PAGE VIEWS</span><strong>${escapeHtml(formatCount(data.totals.pageViews))}</strong></div>
    <div class="card"><span>PRODUCT VIEWS</span><strong>${escapeHtml(formatCount(data.totals.productViews))}</strong></div>
  ...
  ${renderFunnel(data)}
  ${renderTable('Origens', 'Canal C1 agregado. “Cliques / page view” é uma razão de eventos, não taxa de venda.', data.origins, [
    { key: 'channel', label: 'Canal' },
    { key: 'pageViews', label: 'Page views', value: item => formatCount(item.pageViews) },
    { key: 'productViews', label: 'Product views', value: item => formatCount(item.productViews) },
    { key: 'affiliateClicks', label: 'Affiliate clicks', value: item => formatCount(item.affiliateClicks) },
    { key: 'clickRatePct', label: 'Cliques / page view', value: item => formatPercent(item.clickRatePct) },
  ])}
  ${renderTable('Landings', 'Primeira landing preservada na sessão C1. Ordenação prioriza impacto comercial observado.', data.landings, [
  ...
  ${renderTable('Produtos', 'product_id permanece a identidade; o título é resolvido do catálogo canônico quando disponível. CTR usa somente card/related elegíveis M3.1.', data.products, [
    { key: 'product_id', label: 'Produto', value: productName },
    { key: 'productViews', label: 'Product views', value: item => formatCount(item.productViews) },
    { key: 'affiliateClicks', label: 'Affiliate clicks', value: item => formatCount(item.affiliateClicks) },
    { key: 'eligibleCtrPct', label: 'CTR comercial elegível', value: item => formatPercent(item.eligibleCtrPct) },
  ])}
  ${renderTable('Placements', 'Impressões e CTR comerciais; CTR é factual somente nos placements elegíveis card/related M3.1.', data.placements, [
    { key: 'placement', label: 'Placement' },
    { key: 'commercialImpressions', label: 'Impressões', value: item => formatCount(item.commercialImpressions) },
    { key: 'affiliateClicks', label: 'Affiliate clicks', value: item => formatCount(item.affiliateClicks) },
    { key: 'eligibleCtrPct', label: 'CTR elegível', value: item => formatPercent(item.eligibleCtrPct) },
  ])}
  ${renderOpportunities(data, productTitles)}
  <p class="foot">Atualizado em ${escapeHtml(data.generatedAtUtc)} · Rankings limitados a ${TOP_ROWS} linhas · Sem sessão individual, PII, receita, pedido ou comissão.</p>
</main>
</body>
</html>`;
  return html;
...
export async function handleCommercialPanel(request, env, { fetchImpl = runtimeFetch } = {}) {
  ...
  try {
    const [data, productTitles] = await Promise.all([
      loadCommercialDashboard({ accountId, apiToken, fetchImpl }),
      loadCanonicalProductTitles(env?.ASSETS, request.url),
    ]);
    if (data.failures.length) {
      const safe = safeDiagnosticsFromError({ failures: data.failures }, { accountId, apiToken, panelPassword: password });
      console.error('PNM commercial panel partial diagnostics', JSON.stringify({ failures: safe }));
    }
