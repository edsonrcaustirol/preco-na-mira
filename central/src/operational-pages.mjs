...
function nav(active) {
  return CENTRAL_AREAS.map(area => `<a href="${PATHS[area.id]}"${area.id === active ? ' aria-current="page"' : ''}>${escapeHtml(area.label)}</a>`).join('');
}
