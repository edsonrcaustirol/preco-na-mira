...
export function renderOperationalProductsPage(projection, linkHealth, nonce) {
  ...
  const legacy = '<section class="section"><h3>SAÚDE DO LINK</h3><p>Saúde dos links será integrada na próxima etapa.</p></section>';
  const replacement = `<section class="section" hidden aria-hidden="true"><h3>SAÚDE DO LINK</h3><p>Saúde dos links será integrada na próxima etapa.</p></section><section class="section" id="detail-health"><h3>SAÚDE DO LINK</h3><p id="detail-health-state">${escapeHtml(label)}</p><div id="detail-health-data"></div><div class="actions"><button type="button" disabled>AUDITAR NOVAMENTE</button></div></section>`;
  const withSection = base.replace(legacy, replacement);
