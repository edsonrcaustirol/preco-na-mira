...
function options() {
  const official = LINK_HEALTH_CLASSIFICATIONS
    ...
    .join('');
  return `<option value="attention">PRECISA DE ATENÇÃO</option><option value="non-verifiable">${escapeHtml(LINK_HEALTH_NON_VERIFIABLE)}</option><option value="all">TODOS OS RESULTADOS</option>${official}`;
}
...
:root{font-family:Inter,system-ui,sans-serif;color:#eaf2ff;background:#08111f}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:#08111f}.layout{display:grid;grid-template-columns:230px minmax(0,1fr);min-height:100vh}.side{padding:24px 18px;border-right:1px solid #203047}.brand{font-weight:800}.sub,.muted{color:#8ea3bd}.side nav{display:grid;gap:6px;margin-top:24px}.side a{color:#cfe0f5;text-decoration:none;padding:10px;border-radius:8px}.side a[aria-current=page],.side a:hover{background:#14233a}.main{padding:30px;min-width:0;max-width:1300px;width:100%}h1{margin:6px 0}.status{display:flex;gap:7px;flex-wrap:wrap;margin:18px 0}.pill{border:1px solid #34506f;border-radius:999px;padding:5px 8px;font-size:11px}.summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin:18px 0}.metric{border:1px solid #223751;background:#0e1b2d;border-radius:12px;padding:14px}.metric span{display:block;color:#8ea3bd;font-size:11px}.metric strong{display:block;font-size:24px;margin-top:5px}.toolbar{display:flex;gap:10px;align-items:end;flex-wrap:wrap;margin:18px 0}.field{display:grid;gap:5px;min-width:240px}.field label{font-size:11px;color:#8ea3bd}.field select{border:1px solid #2b415f;background:#0e1b2d;color:#eaf2ff;border-radius:9px;padding:10px}.notice,.empty{border:1px dashed #34506f;border-radius:12px;padding:18px;color:#a9bdd4}.results{display:grid;gap:9px}.result-card{display:grid;grid-template-columns:180px minmax(140px,240px) minmax(0,1fr);gap:12px;align-items:center;text-align:left;border:1px solid #223751;background:#0e1b2d;color:#eaf2ff;border-radius:11px;padding:12px}.result-card[hidden],.empty[hidden],.detail[hidden]{display:none}.classification{font-size:11px;border:1px solid #34506f;border-radius:999px;padding:5px 8px;width:max-content;max-width:100%}.reason{color:#9fb8d2;overflow-wrap:anywhere}.detail{margin-top:18px;border:1px solid #2b415f;background:#0b1828;border-radius:14px;padding:18px}.detail-head{display:flex;justify-content:space-between;gap:12px}.close,.actions button{border:1px solid #2b415f;background:#14233a;color:#eaf2ff;border-radius:8px;padding:8px 10px}.row{display:grid;grid-template-columns:120px minmax(0,1fr);gap:10px;padding:7px 0;border-bottom:1px solid #182a41}.row span{color:#7890aa}.row strong{font-weight:500;overflow-wrap:anywhere}.actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:14px}.actions button:disabled{opacity:.45}.legend{margin-top:18px;color:#8ea3bd;font-size:12px;line-height:1.6}@media(max-width:800px){.layout{display:block}.side{border-right:0;border-bottom:1px solid #203047}.side nav{display:flex;overflow:auto}.main{padding:22px 16px}.summary{grid-template-columns:repeat(2,minmax(0,1fr))}.result-card,.row{grid-template-columns:1fr}.field{min-width:100%;width:100%}.field select{width:100%}}
</style></head><body><div class="layout"><aside class="side"><div class="brand">Preço na Mira</div><div class="sub">Central Operacional</div><nav>${nav}</nav></aside><main class="main"><div class="muted">L2.4B · ${escapeHtml(CENTRAL_LINK_HEALTH_CONTRACT)}</div><h1>Saúde dos Links</h1><p class="muted">Leitura segura preparada para resultados do auditor oficial ${escapeHtml(AFFILIATE_INTEGRITY_CONTRACT)}. Nenhuma auditoria é disparada por esta tela.</p><div class="status"><span class="pill">Somente leitura</span><span class="pill">Última referência: ${escapeHtml(runLabel)}</span><span class="pill">Sem D1 nesta etapa</span></div>
<section class="summary" aria-label="Resumo"><div class="metric"><span>PRECISA DE ATENÇÃO</span><strong>${count(summary?.attention)}</strong></div><div class="metric"><span>${escapeHtml(LINK_HEALTH_NON_VERIFIABLE)}</span><strong>${count(summary?.nonVerifiable)}</strong></div><div class="metric"><span>RESULTADOS</span><strong>${count(summary?.total)}</strong></div><div class="metric"><span>ESTADOS OFICIAIS</span><strong>${LINK_HEALTH_CLASSIFICATIONS.length}</strong></div></section>
<div class="toolbar"><div class="field"><label for="health-filter">Mostrar</label><select id="health-filter">${options()}</select></div></div>
${unavailable ? '<div class="notice" id="health-unavailable"><strong>Nenhuma auditoria disponível.</strong><br>Quando o executor oficial produzir resultados, esta área exibirá principalmente exceções sem inventar estado de saúde.</div>' : ''}
<section class="results" id="health-results" aria-live="polite">${cards}</section><div class="empty" id="filtered-empty" hidden>Nenhum resultado corresponde ao filtro selecionado.</div>
<section class="detail" id="health-detail" hidden aria-labelledby="health-detail-title"><div class="detail-head"><div><div class="muted">Detalhe do resultado</div><h2 id="health-detail-title"></h2></div><button class="close" id="health-detail-close" type="button">Fechar</button></div><div id="health-detail-body"></div><div class="actions"><button type="button" disabled>AUDITAR NOVAMENTE</button><button type="button" disabled>INFORMAR NOVO LINK</button></div></section>
<div class="legend">PRECISA DE ATENÇÃO = ${LINK_HEALTH_ATTENTION_CLASSIFICATIONS.map(escapeHtml).join(' · ')}.<br>${escapeHtml(LINK_HEALTH_NON_VERIFIABLE)} fica separado e nunca é tratado automaticamente como link quebrado.</div>
