...
function jsonLd(html) {
  ...
  while ((match = re.exec(html))) {
    try { const value = JSON.parse(match[1]); if (Array.isArray(value)) out.push(...value); else if (value) out.push(value); } catch {}
  }
  const flat = [];
  const visit = value => { if (!value || typeof value !== 'object') return; flat.push(value); if (Array.isArray(value['@graph'])) value['@graph'].forEach(visit); };
  out.forEach(visit);
  return flat;
}
...
function cleanName(value) {
  ...
  if (half > 25) for (let i = Math.max(25, half - 8); i <= Math.min(name.length - 25, half + 8); i++) { const left = name.slice(0, i).trim(); const right = name.slice(i).trim(); if (right.startsWith(left)) { name = left; break; } }
  return name.slice(0, 180).trim();
}
function genericDescription(value) {
  const v = text(value).toLowerCase();
  return !v || /visite a página|todos os produtos|preco na mira|preço na mira|em um só lugar|loja oficial|perfil/.test(v);
}
function fallbackDescription(name) {
  if (/costur/i.test(name)) {
    const points = name.match(/\b(\d{1,3})\s*pontos?\b/i)?.[1];
    const portable = /port[aá]til/i.test(name);
    const parts = [`${name.replace(/\s+Acabamento Profissional$/i, '').trim()}`];
    const features = [];
    if (points) features.push(`${points} pontos`);
    if (portable) features.push('formato portátil');
    features.push('uso doméstico');
    return `${parts[0]}, com ${features.join(', ')}. Indicada para costuras, ajustes e projetos do dia a dia.`.slice(0, 320);
  }
  return `${name}. Confira características, disponibilidade e condições atuais no Mercado Livre.`.slice(0, 320);
}
function cleanDescription(value, name) {
  let description = text(value).replace(/Comprar|Compre|Mercado Livre/gi, '').replace(/\s+/g, ' ').trim();
  if (genericDescription(description) || description.length < 35) description = fallbackDescription(name);
  return description.slice(0, 320).trim();
}
function firstImage(value) { const candidate = Array.isArray(value) ? value[0] : value; if (candidate && typeof candidate === 'object') return text(candidate.url || candidate.contentUrl); return text(candidate); }
...
function inferCategory(name, node, html) {
  if (/costur/i.test(name)) return 'Máquina de costura';
...
export async function enrichMercadoLivreProduct(rawUrl, options = {}) {
  ...
  for (let step = 0; step <= MAX_REDIRECTS; step++) {
    response = await fetchImpl(current.href, { method: 'GET', redirect: 'manual', headers: { accept: 'text/html,application/xhtml+xml', 'user-agent': 'PrecoNaMira-Central/1.0 (+https://preconamira.com.br)' } });
    if ([301,302,303,307,308].includes(response.status)) { const location = response.headers.get('location'); if (!location || step === MAX_REDIRECTS) return { ok: false, code: 'REDIRECT_FAILED' }; current = safeUrl(new URL(location, current).href); continue; }
    break;
  }
